...
kirbytext::$tags['image'] = array(
  ...
  'html' => function($tag) {
    ...
    $link    = $tag->attr('link');
    $file    = $tag->file($url);
    $caption = $tag->attr('caption') ? $tag->attr('caption') : $file->caption();

    // use the file url if available and otherwise the given url
    $url = $file ? $file->url() : url($url);

    if ($tag->page()->parent()->template() == 'news') :
      $url = thumb($file,array('width' => 414, 'height' => 414))->url();
      $url2x = thumb($file,array('width' => 818, 'height' => 818))->url();
    else:
      $url = thumb($file,array('width' => 505, 'height' => 505))->url();
      $url2x = thumb($file,array('width' => 1010, 'height' => 1010))->url();
    endif;
    ...
    // image builder
    $_image = function($class) use($tag, $url, $alt, $title, $url2x) {
